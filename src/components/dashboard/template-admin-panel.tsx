...
export default function TemplateAdminPanel({ heading = 'Template Management', description = 'Create, edit, approve, and delete templates for the marketplace.', search = '', setSearch, page = 1, setPage, templatesPerPage = 10 }: TemplateAdminPanelProps) {
  ...
  return (
    <div className="w-full max-w-5xl mx-auto bg-black border-2 border-gray-700 shadow-xl rounded-2xl p-8 mb-12">
      ...
      {/* AI Code Assistant */}
      <div className="w-full bg-gradient-to-br from-gray-800 via-gray-900 to-gray-800 border border-gray-600 rounded-2xl p-6 mb-6 shadow-sm">
        ...
        <p className="text-gray-300 mb-3 text-sm">Describe what you want to build and get instant code suggestions powered by AI.</p>
        <div className="flex flex-col sm:flex-row gap-3 w-full">
          <textarea
            ...
            rows={2}
            className="flex-1 border border-gray-600 rounded-lg p-2 text-sm text-white bg-gray-800 focus:ring-2 focus:ring-purple-400 focus:outline-none transition"
            style={{ minHeight: 44 }}
          />
          <button
            onClick={handleAIGenerate}
            disabled={aiLoading || !aiPrompt.trim()}
            className="h-11 px-6 rounded-lg bg-gradient-to-r from-purple-600 to-blue-500 text-white font-semibold shadow hover:from-purple-700 hover:to-blue-600 transition disabled:opacity-60 mt-2 sm:mt-0 flex items-center gap-2"
          >
            {aiLoading ? (
              <>
        ...
        {aiCode && (
          <div className="mt-4 bg-gray-900 border border-gray-600 rounded-xl p-4 text-xs font-mono whitespace-pre-wrap relative shadow-sm">
            {/* HTML Block */}
            <div className="mb-4">
              <span className="font-bold text-purple-400 mr-2">HTML:</span>
              <button
              ...
              <pre className="bg-gray-800 border border-gray-600 rounded p-2 mt-2 overflow-x-auto text-white text-xs" style={{minHeight: 40}}>{codeBlocks.html}</pre>
            </div>
            {/* CSS Block */}
            <div className="mb-4">
              <span className="font-bold text-blue-400 mr-2">CSS:</span>
              <button
              ...
              <pre className="bg-gray-800 border border-gray-600 rounded p-2 mt-2 overflow-x-auto text-white text-xs" style={{minHeight: 40}}>{codeBlocks.css}</pre>
            </div>
            {/* JavaScript Block */}
            <div className="mb-4">
              <span className="font-bold text-green-400 mr-2">JavaScript:</span>
              <button
              ...
              <pre className="bg-gray-800 border border-gray-600 rounded p-2 mt-2 overflow-x-auto text-white text-xs" style={{minHeight: 40}}>{codeBlocks.js}</pre>
            </div>
            <div className="flex gap-2 mb-2">
              <button
                onClick={handleCopyAICode}
                className="px-3 py-1 bg-gray-600 text-white rounded text-xs flex items-center gap-1 hover:bg-gray-700 transition"
              >
                {copied ? 'Copied!' : 'Copy'}
              </button>
            </div>
      ...
      <div className="flex flex-col gap-8">
        ...
        <form onSubmit={handleSubmit} className="flex-1 flex flex-col gap-4 bg-gray-900 p-6 rounded-xl shadow border border-gray-700">
          ...
          <div className="flex gap-2 mt-2">
            ...
            {editing && <button type="button" className="btn-secondary px-5 py-2 rounded bg-gray-600 text-white font-bold shadow hover:bg-gray-700 transition" onClick={() => { setEditing(null); setForm({ name: '', category: '', price: 0, description: '', preview: '', pages: requiredPages.reduce((acc, page) => ({ ...acc, [page]: { html: '', css: '', js: '' } }), {}) }); }}>Cancel</button>}
          </div>
        </form>
      </div>
      {/* Table Section */}
